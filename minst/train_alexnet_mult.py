...
for adversarial in range(1, 6):
    for gaussian in range(1, 6):
        # 定义训练数据集
        train_mnist = MyDataset("../data/minst_mult_alexnet_trainset/MINST-2000-{:.0f}-{:.0f}-image.pt".format(2 * adversarial, 2 * gaussian),
                                "../data/minst_mult_alexnet_trainset/MINST-2000-{:.0f}-{:.0f}-labels.pt".format(2 * adversarial, 2 * gaussian))
        # 原始测试集
        ...
        net_path = "../models/mult_alexnet/mult_alexnet_{:.0f}_{:.0f}".format(2 * adversarial, 2 * gaussian)
        ...
        if __name__ == "__main__":
            ...
            for epoch in range(EPOCH):
                ...
                if early_stopping.early_stop and correct1 / total1 > 0.7:
                    ...
                    print(2 * adversarial, 2 * gaussian)
                    break
                else:
