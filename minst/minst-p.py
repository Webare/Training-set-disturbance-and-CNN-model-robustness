...
# torch.save(test_lables, "../data/minst_fgsm_lenet_test_lables.pt")

model = LeNet().to(device)
pretrained_model = "../models/lenet_020.pth"
...
for idx, image in enumerate(train_images):
    all_perturbed_images.append(perturbed_image(image, train_lables[idx]))
    if idx % 100 == 0:
        print(idx, len(train_images))
    # show(image)
    if idx % 5000 == 0:
        torch.save(all_perturbed_images, "../data/minst_fgsm_lenet_trainset_{0}.pt".format(idx))
torch.save(all_perturbed_images, "../data/minst_fgsm_lenet_trainset.pt")

for i in range(25):
    perturbed_images = [j[i] for j in all_perturbed_images]
    torch.save(perturbed_images, "../data/minst_fgsm_lenet_train_images_{:.2f}.pt".format(i/2))
