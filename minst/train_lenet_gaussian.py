...
for idx in range(1, 25):
    # 定义训练数据集
    train_mnist = MyDataset("../data/minst_gaussian_noise_trainset/MINST-20000-{:0.2f}-image.pt".format(idx / 2),
                            "../data/minst_gaussian_noise_trainset/MINST-20000-{:0.2f}-labels.pt".format(idx / 2))
    # 原始测试集
    ...
    net = LeNet().to(device)
    net_path = "../models/gaussian_lenet/gaussian_noise_lenet_{:0.2f}".format(idx / 2)
    criterion = nn.CrossEntropyLoss()  # 交叉熵损失函数，通常用于多分类问题上
    optimizer = optim.SGD(net.parameters(), lr=LR, momentum=0.9)  # 梯度下降法求损失函数最小值
    early_stopping = EarlyStopping(patience=20, verbose=True, path=net_path)
